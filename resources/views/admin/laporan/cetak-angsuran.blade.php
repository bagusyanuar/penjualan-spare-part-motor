...
@section('content')
    <div class="text-center f-bold report-title">Laporan Angsuran</div>
    ...
    <table id="my-table" class="table display f-small">
        ...
        <tr>
            ...
            <th width="15%" class="text-center f-semi-bold">Customer</th>
            <th class="f-semi-bold">Keterangan</th>
            <th width="12%" class="text-right f-semi-bold">Nominal</th>
        </tr>
        </thead>
        <tbody>
        @foreach($data as $v)
            <tr>
                ...
                <td class="f-small middle-header text-center">{{ $v->penjualan->user->customer->nama }}</td>
                <td class="f-small middle-header">Angsuran Ke {{ $v->index }}</td>
